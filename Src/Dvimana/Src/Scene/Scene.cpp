...
namespace Dvimana {
    void Scene::OnUpdate(TimeSteps deltaTime){
        ...
        if(primaryCamera != nullptr){
            BatchRenderer::Begin(*primaryCamera, cameraTransform);
            auto group = m_Registry.group<TransformComponent>(entt::get<SpriteComponent>);
            for(auto entity : group){
                auto [transform, sprite] = group.get<TransformComponent, SpriteComponent>(entity);
                BatchRenderer::Quad(transform.GetTransform(), sprite.Color);
            }
            BatchRenderer::End();
        }
    }
    ...
    void ScenePanels::RenderComponents(Entity entity){
        ...
        if(entity.HasComponent<TransformComponent>()){
            if(ImGui::TreeNodeEx((void*)typeid(TransformComponent).hash_code(), ImGuiTreeNodeFlags_DefaultOpen, "Transform")){
                ...
                Vec3DragController("Scale", tc.Scale);
                ImGui::TreePop();
            }
        } 
